...
def add_course_name_to_numbers(string):
    ...
    for i in range(len(words)):
        ...
        if re.search(r'[A-Z]{3,8}', word):
            current_course = word
            current_course_index = i
        elif i < len(words) - 1 and re.search(r'\d{3}G?', word) and re.search(r'points', words[i + 1]):
            continue
        elif i > current_course_index + 1 and re.search(r'\d{3}G?', word):
...
def create_rule_points_from(string):
    ...
    match = re.match(r'(\d\d points, including at least)', string)
    if match and match.group(1):
        arr = string.split('including at least ')
        return [match.group(1)] + [parse_points_from(arr[1])]
...
def parse_points_from(string):
    ...
    string = re.sub(r'(?:, )(?=\d\d points)', ' and ', string)
    ...
    if re.search(r'^\d\d points(:| from) ([A-Z]{3,8} [1237]\d\dG?(, | or )?)+$', string):
        return create_rule_points_from(string)

    or_array = re.split(r', or ', string)
    if not re.search('either', string) and len(or_array) > 1:
        parsed_or_array = ['or']

        for rule in or_array:
            parsed_or_array.append(parse(rule))

        return parsed_or_array

    and_array = re.split(
        r'(?:, and| and|, and either| and either) (?=\d\d points from|a further \d\d points from|\d\d points:|[A-Z]{3,8} [1237]\d\dG? or|at least \d\d points from)', string)
    parsed_and_array = []

    if len(and_array) == 1:
        or_array_2 = re.split(
            r'(?: or) (?=\d\d points from|a further \d\d points from|\d\d points:|at least \d\d points from)', string)

        if len(or_array_2) > 1:
            parsed_or_array = ['or']
            for rule in or_array_2:
                parsed_or_array.append(parse(rule))
            return parsed_or_array
        return create_rule_points_from(and_array[0])
    ...
    for rule in and_array:
        ...
        # else:
        parsed_and_array.append(parse(rule))
...
def parse_points_at_stage(string):
    ...
    and_array = re.split(
        r'(?:;| and)? (?=\d\d points (?:passed at|at|from other) Stage)|(?: and) (?=[A-Z]{3,8} \d{3})', string)
    ...
    or_array2 = re.split(r'(?: or )(?=[A-Z]{3,8} \d{3})', string)
    ...
    if match:
        rule = f'{match.group(1)}:{match.group(2)}'  # e.g. "30:Stage II"
...
def parse_grade(string):
    ...
    if len(or_rules) == 1:
        or_rules = re.split(r' or ?(?=(?:a )?(?:[ABCD](?:\+|\-)?|\d out of \d) )', string)
    ...
    and_rules = re.split(r'(?: and )(?=a [ABCD]\+|\-)', string)
    ...
    match = re.match(
        r'(?:a |at least )?([ABCD](?:\+|\-)?)(?: grade)?(?: or higher)? in(?: either| both)? (.*)', string)
...
def parse_reverse_grade(string):
    match = re.match(r'([A-Z]{3,8} \d{3}) (?:passed with a grade of|with a)? ([ABC](?:\+|\-)?)(?: or higher)?', string)
...
def parse_grade_stage(string):
    match = re.match(r'([ABC](?:\+|\-)?) average or higher at (Stage I{1,3}) in (.*)', string)

    if match and len(match.groups()) == 3:
        return [match.group(1), match.group(2), parse_prereq(match.group(3))]
...
def parse_cannot_be_taken_with_or_after(string):
    ...
    match = re.search(
        r'Cannot be taken at the same time as any other (.*?) course, or after any successfully completed (.*?) course, other than ([A-Z]{3,8} [1237]\d\dG?), ([A-Z]{3,8} [1237]\d\dG?)', string)
...
def parse_may_not_be_taken_after(string):
    ...
    string = re.sub(r'([A-Z]{3,8} [1237]\d\d )?may not be taken after any ', '', string)
...
def parse_may_not_be_taken_after_mix(string):
    ...
    match1 = re.match(
        r'[A-Z]{3,8} [1237]\d\d may not be taken concurrently with any other (.*?) course, except ([A-Z]{3,8} [1237]\d\d)', and_array[0])
    ...
    match2 = re.match(
        r'may not be taken after ([A-Z]{3,8} [1237]\d\dG?) or any (.*?) course at Stage I or above, except ([A-Z]{3,8} [1237]\d\dG?), ([A-Z]{3,8} [1237]\d\dG?)', and_array[1])
...
def parse_must_enrol_in(string):
    ...
    match = re.match(
        r'To complete this course students must enrol in ([A-Z]{3,8} [1237]\d\d) A and B, or ([A-Z]{3,8} [1237]\d\d)', string)
...
def and_or_split(string):
    ...
    if re.search(r'^((, or |, | or )?(?! and )[A-Z]{3,8} [1237]\d\d)*$', string) and re.search(' or ', string):
...
def parse_prereq(string):
    ...
    if same_string_check == string:
        ...
        if string_counter > 10:
            print('Error Rule: ', string)
            raise Exception('Error: Same string')
    else:
        string_counter = 0
    same_string_check = string

    if re.search(r'^a further \d\d points from [A-Z]{3,8}', string):
        return parse_further_points_from(string)
    if re.search(r'^\d\d points( from|:) [A-Z]{3,8}', string):
    ...
    if re.search(r'[ABCD](\+|\-)?( grade)?( or higher)? in (both )?(either|[A-Z]{3,8} \d{3}|CIE)', string):
        return parse_grade(string)
    if re.search(r'[A-Z]{3,8} \d{3} (passed with a grade of|with a) [ABC](\+|\-)?( or higher)?', string):
        return parse_reverse_grade(string)
    if re.search(r'[ABC](\+|\-)? average or higher at Stage I{1,3} in', string):
        return parse_grade_stage(string)
...
def wrap_with_array(prereq):
    if isinstance(prereq, str):
        if re.search(r'^[A-Z]{3,8} [1237]\d\dG?$', prereq):
            return ['and', prereq]
        else:
            return [prereq]
    return prereq
